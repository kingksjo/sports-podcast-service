...
4. **Metadata Decisions:** The Cloud Run service currently does *not* write a `_podcast_meta.json` sidecar. If the frontend requires metadata display (like sport, match title, or overview), this functionality must be explicitly added to `storage_actions.py` in the backend before building dependent frontend components.
